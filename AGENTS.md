...
| `xanad-lifecycle-planning` | yes | `inspect`, `check`, `plan`, `apply`, `update`, `repair`, and `factory-restore` for xanad-assistant-managed surfaces |
...
- `xanad-lifecycle-planning` may delegate to `Explore` for repo inventory, `Debugger` for failing lifecycle behavior, and `Planner` for phased remediation.
...
| `xanad-lifecycle-planning` | `Debugger` | `inspect`, `check`, `plan`, `apply`, `update`, or `repair` results are surprising or failing and the control path is unclear |
| `xanad-lifecycle-planning` | `Planner` | Repair, update, migration, or factory-restore work needs phased remediation before execution |

## Lifecycle Trigger Phrases

| Trigger phrase | Route |
|---|---|
| `inspect workspace` | `xanad-lifecycle-planning` |
| `run lifecycle check` | `xanad-lifecycle-planning` |
| `repair install` | `xanad-lifecycle-planning` |
| `update xanad-assistant` | `xanad-lifecycle-planning` |
| `factory restore` | `xanad-lifecycle-planning` |
