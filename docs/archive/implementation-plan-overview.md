...
This document turns the architecture in `docs/lifecycle-update-plan.md` (archived — superseded by `docs/contracts/`) into an implementation sequence.
...
Use `docs/lifecycle-update-plan.md` (archived — superseded by `docs/contracts/`) for:
...
- Phase 7 is complete: `agents/lifecycle-planning.agent.md` (archived — succeeded by `agents/xanadLifecycle.agent.md`) now includes trigger phrases, a full command reference with exact CLI invocations for all lifecycle commands including GitHub source and dry-run, and a clear responsibility boundary. `template/prompts/setup.md` provides a complete five-step workflow (Inspect → Clarify → Plan → Apply → Confirm) with concrete command invocations and dry-run guidance.
