...
- Naming convention: **camelCase** for consumer-facing multi-word names (MCP server scripts like `gitMcp.py`, agent/skill directory names like `lifecycleAudit/`); **underscores** for Python-importable modules (lifecycle engine submodules like `_plan_a.py`, MCP helpers like `_xanad_mcp_source.py`).
