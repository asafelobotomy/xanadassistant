...
class XanadAssistantInspectTests(unittest.TestCase):
    ...
    def test_check_detects_clean_and_unmanaged_targets(self) -> None:
        def workspace_setup(workspace: Path, repo_root: Path) -> None:
            ...
            target_two.write_text((repo_root / "template" / "prompts" / "setup.md").read_text(encoding="utf-8"), encoding="utf-8")
    ...
    def test_plan_setup_classifies_unmanaged_lookalikes(self) -> None:
        def workspace_setup(workspace: Path, repo_root: Path) -> None:
            ...
            managed_prompt.write_text((repo_root / "template" / "prompts" / "setup.md").read_text(encoding="utf-8"), encoding="utf-8")
    ...
    def test_plan_repair_rejects_clean_installed_state(self) -> None:
        def workspace_setup(workspace: Path, repo_root: Path) -> None:
            ...
            prompt.parent.mkdir(parents=True, exist_ok=True)
            prompt.write_text(
                (repo_root / "template" / "prompts" / "setup.md").read_text(encoding="utf-8"),
                encoding="utf-8",
            )
