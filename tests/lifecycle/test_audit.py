...
from scripts.lifecycle._xanad._audit import (
...
def _patch_all(context: dict | None = None, check: dict | None = None, source: dict | None = None):
    ...
    return mock.patch.multiple(
        "scripts.lifecycle._xanad._audit",
...
class AuditReportFieldsTests(unittest.TestCase):
    def test_build_audit_report_returns_required_top_level_keys(self) -> None:
        with _patch_all():
            report = build_audit_report(Path("."), _PKG_ROOT)

        for key in ("auditSchemaVersion", "generatedAt", "package", "install", "check", "system",
                    "issueTitle", "issueBody", "issueLabels"):
            with self.subTest(key=key):
                self.assertIn(key, report)

    def test_build_audit_report_schema_version_matches_constant(self) -> None:
        with _patch_all():
            report = build_audit_report(Path("."), _PKG_ROOT)
        self.assertEqual(report["auditSchemaVersion"], AUDIT_SCHEMA_VERSION)

    def test_build_audit_report_includes_package_version(self) -> None:
        with _patch_all():
            report = build_audit_report(Path("."), _PKG_ROOT)
        self.assertEqual(report["package"]["version"], "0.2.0")

    def test_build_audit_report_includes_install_metadata(self) -> None:
        with _patch_all():
            report = build_audit_report(Path("."), _PKG_ROOT)
    ...
    def test_build_audit_report_includes_check_status(self) -> None:
        with _patch_all():
            report = build_audit_report(Path("."), _PKG_ROOT)
        self.assertEqual(report["check"]["status"], "clean")

    def test_build_audit_report_includes_system_info(self) -> None:
        with _patch_all():
            report = build_audit_report(Path("."), _PKG_ROOT)
    ...
    def test_build_audit_report_preserves_optional_label(self) -> None:
        with _patch_all():
            report = build_audit_report(Path("."), _PKG_ROOT, label="my-project")
        self.assertEqual(report["label"], "my-project")

    def test_build_audit_report_label_none_by_default(self) -> None:
        with _patch_all():
            report = build_audit_report(Path("."), _PKG_ROOT)
        self.assertIsNone(report["label"])

    def test_build_audit_report_issue_labels_contains_audit_report(self) -> None:
        with _patch_all():
            report = build_audit_report(Path("."), _PKG_ROOT)
        self.assertIn("audit-report", report["issueLabels"])


class AuditSourceSanitizationTests(unittest.TestCase):
...
class AuditWarningsTests(unittest.TestCase):
...
class AuditIssueTitleTests(unittest.TestCase):
    def test_issue_title_includes_version_and_status(self) -> None:
        ...
        self.assertTrue(title.startswith("[Audit]"))


class AuditIssueBodyTests(unittest.TestCase):
    def test_issue_body_includes_all_sections(self) -> None:
        with _patch_all():
            report = build_audit_report(Path("."), _PKG_ROOT)
        body = report["issueBody"]
        for section in ("Package", "Install State", "Check", "System", "Audit Metadata"):
    ...
    def test_issue_body_includes_label_when_provided(self) -> None:
        with _patch_all():
            report = build_audit_report(Path("."), _PKG_ROOT, label="acme-corp")
        self.assertIn("acme-corp", report["issueBody"])


class AuditResultPayloadTests(unittest.TestCase):
    def test_build_audit_result_returns_command_field(self) -> None:
        with _patch_all():
            payload = build_audit_result(Path("."), _PKG_ROOT)
        self.assertEqual(payload["command"], "audit")

    def test_build_audit_result_status_is_ok(self) -> None:
        with _patch_all():
            payload = build_audit_result(Path("."), _PKG_ROOT)
        self.assertEqual(payload["status"], "ok")

    def test_build_audit_result_result_contains_audit_report(self) -> None:
        with _patch_all():
            payload = build_audit_result(Path("."), _PKG_ROOT)
        self.assertIn("auditSchemaVersion", payload["result"])


class AuditCLIIntegrationTests(unittest.TestCase):
    """Verify the audit subcommand is reachable via the CLI."""

    def test_audit_subcommand_help_exits_cleanly(self) -> None:
        result = subprocess.run(
            [sys.executable, "xanadAssistant.py", "audit", "--help"],
            capture_output=True,
            text=True,
            cwd=str(_REPO_ROOT),
        )
    ...
    def test_audit_subcommand_requires_workspace(self) -> None:
        result = subprocess.run(
            [sys.executable, "xanadAssistant.py", "audit"],
            capture_output=True,
            text=True,
            cwd=str(_REPO_ROOT),
        )
