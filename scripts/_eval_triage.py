"""Triage agent eval: treatment (full agent prompt) vs control (bare model).

Loads the triage agent system prompt from agents/triage.agent.md,
resolves pack tokens from packs/core/tokens.json, then runs each task
from _eval_tasks.py against both the treatment arm and a control arm
(no system prompt). Scores each response via _eval_judge.py.
"""
...
_AGENT_FILE = REPO_ROOT / "agents" / "triage.agent.md"
_CORE_TOKENS = REPO_ROOT / "packs" / "core" / "tokens.json"

# Default model: fast, cost-efficient, broadly available on GitHub Models.
...
def load_system_prompt() -> str:
    """Return the triage agent body with pack tokens resolved from core pack."""
...
def run(model: str = DEFAULT_MODEL) -> dict:
    """Run all triage tasks: treatment vs control. Returns full results dict."""
    # Import here so offline unit tests can import this module without triggering
    # the API client import.
    import _eval_client as _client
    import _eval_judge as _judge
    import _eval_tasks as _tasks

    system_prompt = load_system_prompt()
    task_results = []

    for task in _tasks.TRIAGE_TASKS:
...
def print_results(data: dict) -> int:
    ...
    tasks = data["tasks"]
    hdr = (f"  {'Task':<28} {'Arm':<12} {'Tier':<10} "
           f"{'Fmt':>3} {'Tie':>3} {'Blk':>3} {'Scr':>3}  "
           f"{'P-tok':>5} {'C-tok':>5}  {'ms':>6}")
    div = "  " + "─" * (len(hdr) - 2)
    print(f"\n=== TRIAGE EVAL — treatment vs control  (model: {data['model']}) ===")
    print(hdr)
    print(div)
    ...
    for entry in tasks:
        for arm in ("treatment", "control"):
            d     = entry[arm]
            sc    = d["score"]           # dict from _eval_judge.score()
            tier  = d["parsed"]["tier"] if d["parsed"] else "—"
            fv    = "✓" if sc["format_valid"]   else "✗"
            tc    = "✓" if sc["tier_correct"]   else "✗"
            bc    = "✓" if sc["blocker_correct"] else "✗"
            pts   = d["prompt_tokens"]
            cts   = d["completion_tokens"]
            lat   = d["latency_ms"]
            print(f"  {entry['task']:<28} {arm:<12} {tier:<10} "
                  f"{fv:>3} {tc:>3} {bc:>3} {sc['score']:>3}  "
...
def save_results(data: dict) -> Path:
    ...
    out = RESULTS_DIR / f"eval-triage-{ts}.json"
