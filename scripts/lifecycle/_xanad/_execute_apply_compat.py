...
def validate_relative_plan_path(path_value: object, field: str) -> str:
...
def validate_apply_plan_paths(
    ...
    *,
    ...
    for index, entry in enumerate(backup_plan.get("targets", [])):
        ...
        backup_path = validate_relative_plan_path(entry.get("backupPath"), f"result.backupPlan.targets[{index}].backupPath")
    ...
    for index, entry in enumerate(backup_plan.get("archiveTargets", [])):
        target = validate_relative_plan_path(entry.get("target"), f"result.backupPlan.archiveTargets[{index}].target")
        archive_path = validate_relative_plan_path(entry.get("archivePath"), f"result.backupPlan.archiveTargets[{index}].archivePath")
        if target not in action_targets:
            raise LifecycleCommandError(
                "contract_input_failure",
...
def load_apply_plan(plan_path: str | None, workspace: Path) -> dict:
    ...
    planned_lockfile = payload.get("result", {}).get("plannedLockfile")
    if planned_lockfile is None:
        raise LifecycleCommandError(
            "contract_input_failure",
...
def validate_apply_plan_package(
    plan_payload: dict,
    package_root: Path,
    *,
